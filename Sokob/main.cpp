...
public:
    ...
    {
        if(sData[y+2*dy][x+2*dx] == '#' || sData[y+2*dy][x+2*dx] == '!' || data[y+2*dy][x+2*dx] != ' ')
            return false;
...
int main()
{
    ...
    cout << level << endl << endl << b.solve() << endl << endl;
    
    float seconds = clock() / CLOCKS_PER_SEC;
    cout<< "programme takes " << seconds << " seconds" << endl << endl;
